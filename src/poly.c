...
Poly PolyCompose(const Poly *p, unsigned count, const Poly x[])
{
    ...
    else
    {
        ...
        Poly resultPoly = PolyFromCoeff(0); //bedziemy dodawac do tego wielomianu wszystkie wielomiany stworzone z jednomianow z listy

        for (unsigned i = 0; i < p->size; i++) //iteruje po jednomianach w liscie
        {
            Poly monoCompose = PolyCompose(&(((p->arr) + i)->p), count - 1, x + 1); //przesuwam indeks tablicy zmniejszajac count, podaje do PolyCompose wielomian
            ...
            resultPoly = PolyAdd(&resultPoly, &polyComposeFromMono); //dodaje do wynikowego wielomianu tego ktory został policzony w danej iteracji
        }
